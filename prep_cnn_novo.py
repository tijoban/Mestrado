...
cpu_percent  → IQR + CNN Autoencoder (sem MinMaxScaler)
mem_percent  → IQR + MinMaxScaler
disk_percent → IQR + MinMaxScaler
Salva: infra_cnn_novo.csv + tabela infra_cnn_novo.
"""
...
OUTPUT_CSV    = "infra_cnn_novo.csv"
TABLE_OUT     = "infra_cnn_novo"
FREQ          = "30min"
IQR_FACTOR    = 1.5
METRICAS      = ["cpu_percent", "mem_percent", "disk_percent"]
CPU_METRICA   = "cpu_percent"
...
def _build_cnn_ae(window):
    inp = layers.Input(shape=(window, 1))
    x   = layers.Conv1D(CNN_FILTERS,      CNN_KERNEL,
                        activation="relu", padding="same")(inp)
    x   = layers.Conv1D(CNN_FILTERS // 2, CNN_KERNEL,
                        activation="relu", padding="same")(x)
    x   = layers.MaxPooling1D(2)(x)
    x   = layers.Conv1D(CNN_FILTERS // 4, CNN_KERNEL,
                        activation="relu", padding="same")(x)
    ...
    x   = layers.Conv1D(CNN_FILTERS,      CNN_KERNEL,
                        activation="relu", padding="same")(x)
    out = layers.Conv1D(1, 1, activation="sigmoid")(x)
...
def cnn_stabilize(s):
    ...
    if smax == smin:
        return s
    normed = (vals - smin) / (smax - smin)
    n      = len(normed)
    if n < CNN_WINDOW + 1:
        return s
    X = np.array([normed[i:i + CNN_WINDOW]
                  for i in range(n - CNN_WINDOW + 1)])
    X = X.reshape(-1, CNN_WINDOW, 1)
    model = _build_cnn_ae(CNN_WINDOW)
    cb    = callbacks.EarlyStopping(patience=CNN_PATIENCE,
                                    restore_best_weights=True)
    model.fit(X, X, epochs=CNN_EPOCHS, batch_size=CNN_BATCH,
              validation_split=0.1, callbacks=[cb], verbose=0)
    preds         = model.predict(X, verbose=0).reshape(-1, CNN_WINDOW)
    reconstructed = np.zeros(n)
    counts        = np.zeros(n)
    for i, pred in enumerate(preds):
        reconstructed[i:i + CNN_WINDOW] += pred
        counts[i:i + CNN_WINDOW]        += 1
    counts = np.where(counts == 0, 1, counts)
    result = reconstructed / counts * (smax - smin) + smin
    return pd.Series(result, index=s.index, name=s.name)
...
def main():
    ...
    for label, df_raw, entidades in [
        ("Nutanix", df_ntx, NUTANIX_ENTIDADES),
        ("vCenter", df_vc,  VCENTER_ENTIDADES),
    ]:
        print(f"\n--- {label} ---")
        for level, name, prefixo in entidades:
            for metrica in METRICAS:
                s = extrair_serie(df_raw, level, name,
                                  metrica, prefixo, full_index)
                if metrica == CPU_METRICA:
                    print(f"  [CNN]      {s.name}")
                    s = cnn_stabilize(s)
                else:
                    print(f"  [Baseline] {s.name}")
                    s = baseline_stabilize(s)
